...
class MyApplication(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.user_info = FoodComputer(user_height, user_weight, target_weight, target_time, tdee)
    ...
    def take_photo(self):
        ...
        if ret:
            ...
            self.ui.listWidget.addItem(f'{print_list[0]:<13}{print_list[1]:>10}{print_list[2]:>10}')
            for food in self.scan_list:    
                self.sql.sql_connect()
                self.sql.cursor.execute(f"select `Product`, `Calories`, `Carbohydrate`, `Protein`, `Fat`, `Sodium`, `Sugar` from `food_list` where `ID` = '{food}';")
    ...
    def add_to_database(self):
        ...
        formatted_date = current_datetime.strftime("%Y,{:d},{:d}".format(current_datetime.month, current_datetime.day))
    ...
    def check_list(self):
        ...
        self.ui.listWidget_2.addItem(f'{check_date:<10} :{Nutrients[0]:>9}{Nutrients[1]:>7}{Nutrients[2]:>7}{Nutrients[3]:>7}{Nutrients[4]:>7}{Nutrients[5]:>8}') 
        self.ui.listWidget_2.addItem(f'{print_line}')
        total = [0, 0, 0, 0, 0, 0]
        for i in f:
            self.ui.listWidget_2.addItem(f'{i[0]:<15}{i[1]:>7.1f}{i[2]:>7.1f}{i[3]:>7.1f}{i[4]:>7.1f}{i[5]:>8.1f}{i[6]:>7.1f} ')
            for j in range(1,7):
                total[j-1] += float(i[j])
        self.ui.listWidget_2.addItem(f'{print_line}')
        ...
        self.ui.listWidget_3.addItem(f'Today :{Nutrients[0]:>14}{Nutrients[1]:>7}{Nutrients[2]:>7}{Nutrients[3]:>7}{Nutrients[4]:>7}{Nutrients[5]:>8}') 
        self.ui.listWidget_3.addItem(f'{print_line}')
        total = [0, 0, 0, 0, 0, 0]
        for i in f:
            self.ui.listWidget_3.addItem(f'{i[0]:<15}{i[1]:>7.1f}{i[2]:>7.1f}{i[3]:>7.1f}{i[4]:>7.1f}{i[5]:>8.1f}{i[6]:>7.1f} ')
            for j in range(1,7):
                total[j-1] += float(i[j])
        self.ui.listWidget_3.addItem(f'{print_line}')
